...
let rec string_of_expval = function
  ...
  | ListVal ls -> "ListVal["^String.concat ";" (List.map string_of_expval ls)^"]"
  | TreeVal Node(a, l, r) -> "TreeVal("^ string_of_expval a ^ (string_of_expval (TreeVal l))^", "^(string_of_expval (TreeVal r)) ^ ")"
  | TreeVal Empty -> "TreeVal(Empty)"
  | _ -> failwith"Not implemented"
  (* | RecordVal fs -> "RecordVal[" ^ String.concat "; " (List.map string_of_expval fs) ^ "]" *)
...
let rec mem (id1, value) l =
  match l with
  | [] -> false
  | (id2, _)::t ->
    if id1 = id2
    then true
    else mem (id1, value) t
